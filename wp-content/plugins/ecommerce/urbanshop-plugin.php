...
function esgi_display_product_collections(): void {
    ...
    echo '<span style="font-size:.85rem;color:var(--color-muted,#6b7280);font-weight:600;">' . esc_html__( 'Collection : ', 'esgi-plugin' ) . '</span>';
...
function esgi_add_to_cart_text( string $text, WC_Product $product ): string {
    if ( ! $product->is_in_stock() ) {
        return __( '⏳ Rupture de stock', 'esgi-plugin' );
    }
    if ( $product->is_type( 'simple' ) ) {
        return __( '🛒 Ajouter au panier', 'esgi-plugin' );
    }
    return $text;
}
...
// Shortcode [esgi_lookbook limit="6" columns="3" collection="slug"]
function esgi_lookbook_shortcode( array $atts ): string {
    ...
    if ( ! $query->have_posts() ) {
        return '<p>' . esc_html__( 'Aucun look disponible.', 'esgi-plugin' ) . '</p>';
    }
...
function esgi_settings_page(): void {
    ...
    if ( isset( $_POST['esgi_settings_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['esgi_settings_nonce'] ) ), 'esgi_save_settings' ) ) {
        ...
        echo '<div class="notice notice-success"><p>' . esc_html__( 'Paramètres sauvegardés.', 'esgi-plugin' ) . '</p></div>';
    }

    $new_days = get_option( 'esgi_new_product_days', 30 );
    ?>
    <div class="wrap">
        <h1><?php esc_html_e( 'Paramètres ESGI', 'esgi-plugin' ); ?></h1>
        <form method="post">
            <?php wp_nonce_field( 'esgi_save_settings', 'esgi_settings_nonce' ); ?>
            <table class="form-table">
                <tr>
                    <th><?php esc_html_e( 'Badge "Nouveau" (jours)', 'esgi-plugin' ); ?></th>
                    <td>
                        <input type="number" name="esgi_new_product_days" value="<?php echo absint( $new_days ); ?>" min="1" max="365" class="small-text">
                        <p class="description"><?php esc_html_e( 'Affiche le badge sur les produits créés dans ce délai.', 'esgi-plugin' ); ?></p>
                    </td>
                </tr>
            </table>
            <?php submit_button(); ?>
        ...
        <hr>
        <h2><?php esc_html_e( 'Endpoints REST', 'esgi-plugin' ); ?></h2>
        <ul>
